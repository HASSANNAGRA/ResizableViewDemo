...
class RKUserResizableView:UIView {
    ...
    var anchorPont = RKUserResizableViewAnchorPoint()
    ...
    func anchorPont(forTouchLocation touchPoint: CGPoint) -> RKUserResizableViewAnchorPoint {
    ...
    func isResizing() -> Bool {
        return (anchorPont.adjustsH != 0 || anchorPont.adjustsW != 0 || anchorPont.adjustsX != 0 || anchorPont.adjustsY != 0)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            anchorPont = anchorPont(forTouchLocation: touch.location(in: self))
            print(anchorPont)
    ...
    func resize(usingTouchLocation touchPoint: CGPoint) {
        ...
        var deltaW: CGFloat = anchorPont.adjustsW * (touchStart.x - touchPoint.x)
        let deltaX: CGFloat = anchorPont.adjustsX * (-1.0 * deltaW)
        var deltaH: CGFloat = anchorPont.adjustsH * (touchPoint.y - touchStart.y)
        let deltaY: CGFloat = anchorPont.adjustsY * (-1.0 * deltaH)
...
class GripViewBorderView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let colors: [CGFloat] = [0.0,0.0,0.0]
        ...
        let allPoints: [CGRect] = [upperLeft, upperRight, lowerRight, lowerLeft]
        for i in 0..<4 {
